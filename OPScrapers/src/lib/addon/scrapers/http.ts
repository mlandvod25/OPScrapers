...
async function resolveTmdbId(query: MediaQuery): Promise<number | null> {
  ...
  const data = await fetchJson<{
    ...
    `https://api.themoviedb.org/3/find/${imdb}?api_key=b3556f3b206e16f82df4d1f6fd4545e6&external_source=imdb_id`,
    4000,
  );
  if (!data) return null;
    ...
    providers.map(async (provider) => {
      try {
        ...
        for (const src of data?.data?.sources ?? []) {
          ...
          seen.add(url);
          hits.push({
            scraperId: "rivestream",
            scraperName: "RiveStream",
            title: `[Rive - ${src.source ?? provider}] ${src.quality ?? "Auto"}`,
            url,
...
function cineSuMasterUrl(tmdbId: number, season?: number, episode?: number): string {
  ...
  for (let i = 0; i < sArr.length; i++) {
    acc ^= keyBytes[i % keyBytes.length];
    acc = ab((acc + ls[i % ls.length] + Math.imul(2654435761, i)) >>> 0);
    sArr[i] = acc & 255;
  }

  const out = new Uint8Array(a.length + 2);
  out[0] = a.length & 255;
  out[1] = (a.length >> 8) & 255;
  let o = (2654435769 ^ a.length) >>> 0;
  for (let i = 0; i < a.length; i++) {
    o = ab((o + sArr[i % sArr.length] + ls[i % ls.length] + i) >>> 0);
    out[i + 2] = (a[i] ^ (o & 255)) ^ sArr[(7 * i + 3) % sArr.length];
  }

  let b64 = "";
  for (let i = 0; i < out.length; i += 3) {
    const n = out[i];
    const a2 = i + 1 < out.length ? out[i + 1] : null;
    const s = i + 2 < out.length ? out[i + 2] : null;
    b64 += wa[n >> 2];
    b64 += wa[((3 & n) << 4) | ((a2 ?? 0) >> 4)];
    if (a2 == null) break;
    b64 += wa[((15 & a2) << 2) | ((s ?? 0) >> 6)];
    if (s == null) break;
    b64 += wa[63 & s];
  }
...
export async function scrapeCineSu(query: MediaQuery): Promise<HttpHit[]> {
  const tmdbId = await resolveTmdbId(query);
  if (!tmdbId) return [];
  const url = cineSuMasterUrl(tmdbId, query.season, query.episode);
  return [
    {
      scraperId: "cinesu",
      scraperName: "CineSu",
...
export async function scrapeVidLink(query: MediaQuery): Promise<HttpHit[]> {
  ...
  const enc = await fetchJson<{ status?: number; result?: string }>(
  ...
  const isTv = query.kind !== "movie";
  const endpoint = isTv
    ? `https://vidlink.pro/api/b/tv/${enc.result}/${query.season ?? 1}/${query.episode ?? 1}`
    : `https://vidlink.pro/api/b/movie/${enc.result}`;

  const data = await fetchJson<{
    stream?: { playlist?: string; qualities?: Record<string, { url?: string }> };
  }>(endpoint, 6000);
  ...
  for (const q of ["1080", "720", "480"]) {
    const u = data.stream.qualities?.[q]?.url;
    if (u) {
      hits.push({
        scraperId: "vidlink",
        scraperName: "VidLink",
...
export async function scrapeA111477(query: MediaQuery): Promise<HttpHit[]> {
  ...
  const host = "https://a.111477.xyz/";
  const config = `${host}::sort=file-desc::limit=3`;
  ...
  const data = await fetchJson<{
    streams?: Array<{ url?: string; title?: string; name?: string }>;
  }>(`${base}${path}`, 8000);
  ...
  for (const s of data.streams) {
    ...
    hits.push({
      scraperId: "a111477",
      scraperName: "111477",
      title: s.title || s.name || "111477 · Direct",
      url,
      quality: "Auto",
...
export async function scrapeVidSrc(query: MediaQuery): Promise<HttpHit[]> {
  ...
  if (isTv) {
    params.set("season", String(query.season ?? 1));
    params.set("episode", String(query.episode ?? 1));
  }

  const data = await fetchJson<{
    status_code?: string | number;
    data?: { stream_urls?: string[] };
  }>(`https://data.vidsrcme.ru/api.php?${params}`, 5000);
